...
socket.on('usuario', manejarUsuario);

async function manejarUsuario(_usuario){
    // busco la plantilla del servidor
    const recursoRemoto = await fetch('plantillas/login.hbs')
    ...
    // relleno la plantilla con los productos recibidos
    const html = functionTemplate({ _usuario })
